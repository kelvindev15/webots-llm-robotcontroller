...
class LLMRobotController:
    ...
    async def ask(self, prompt: str, maxIterations: int = 30):
        ...
        if controlAcquired:
            ...
            self.eventManager.notify(EventType.SIMULATION_STARTED, {"model": self.chat.model_name, "prompt": prompt, "experiment_id": chat_id })
            iterations = 0
            try:
                self.eventManager.notify(EventType.SENDING_MESSAGE_TO_LLM, { "message": prompt, "withImage": True })
                ...
                while iterations < maxIterations and not(response.ok and response.value.command == "COMPLETE"):
                    ...
                    if response.ok and self.actionAdapter.checkSafety(response.value, self.robot.getLidarImage(atDegree=90, fov=30)): # robot action
                        self.eventManager.notify(EventType.LLM_EXECUTING_ROBOT_ACTION, { "action": response.value })
                        actionResult = await self.actionAdapter.execute(response.value)
                        if actionResult.status == ActionStatus.SUCCESS:
                            ...
                            self.eventManager.notify(EventType.SENDING_MESSAGE_TO_LLM, { "message": self.__buildSceneDescription(), "withImage": True })
                        ...
                        self.eventManager.notify(EventType.SENDING_MESSAGE_TO_LLM, { "message": message, "withImage": True })
                ...
                if iterations >= maxIterations:
                    ...
                    self.eventManager.notify(EventType.SIMULATION_ABORTED, {})
